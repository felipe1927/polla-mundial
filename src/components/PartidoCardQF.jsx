...
function PartidoCardQF({
  ...
  // pronosticosQF (prop del padre) es la única fuente de verdad.
  ...
  const pick = pronosticosQF?.[partido.id] ?? { local: "", visitante: "" }

  // Lógica para estados (PROXIMAMENTE, EN VIVO, etc)
  const ahora = new Date()
  const fechaPartido = new Date(partido.hora)
  const fechaLimitePronostico = new Date(fechaPartido.getTime() - 5 * 60000) // 5 mins antes
  ...
  const handleAdminLocalChange = (e) => {
    const val = e.target.value.replace(/[^0-9]/g, "")
    const nuevoLocal = val === "" ? "" : parseInt(val, 10)
    const visitante = marcadoresQF?.[partido.id]?.visitante ?? ""
  ...
  const handleAdminVisitanteChange = (e) => {
    const val = e.target.value.replace(/[^0-9]/g, "")
    const nuevoVisitante = val === "" ? "" : parseInt(val, 10)
    const local = marcadoresQF?.[partido.id]?.local ?? ""
  ...
  const marcador = marcadoresQF?.[partido.id]
...
function areEqual(prev, next) {
  ...
  return (
    ...
    prev.enviadoQF === next.enviadoQF &&
    prev.qfCerrado === next.qfCerrado &&
    (prev.pronosticosQF?.[id]?.local ?? "") === (next.pronosticosQF?.[id]?.local ?? "") &&
    (prev.pronosticosQF?.[id]?.visitante ?? "") === (next.pronosticosQF?.[id]?.visitante ?? "") &&
    (prev.marcadoresQF?.[id]?.local ?? "") === (next.marcadoresQF?.[id]?.local ?? "") &&
    (prev.marcadoresQF?.[id]?.visitante ?? "") === (next.marcadoresQF?.[id]?.visitante ?? "")
  )
}

export default React.memo(PartidoCardQF, areEqual)
