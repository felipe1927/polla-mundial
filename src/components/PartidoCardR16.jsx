...
function PartidoCardR16({
  ...
  const handleAdminLocalChange = (e) => {
    const val = e.target.value.replace(/[^0-9]/g, "")
    const nuevoLocal = val === "" ? "" : parseInt(val, 10)
    const visitante = marcadoresR16?.[partido.id]?.visitante ?? ""
  ...
  const handleAdminVisitanteChange = (e) => {
    const val = e.target.value.replace(/[^0-9]/g, "")
    const nuevoVisitante = val === "" ? "" : parseInt(val, 10)
    const local = marcadoresR16?.[partido.id]?.local ?? ""
  ...
  const marcador = marcadoresR16?.[partido.id]
...
function areEqual(prev, next) {
  ...
  return (
    ...
    prev.enviadoR16 === next.enviadoR16 &&
    prev.r16Cerrado === next.r16Cerrado &&
    (prev.pronosticosR16?.[id]?.local ?? "") === (next.pronosticosR16?.[id]?.local ?? "") &&
    (prev.pronosticosR16?.[id]?.visitante ?? "") === (next.pronosticosR16?.[id]?.visitante ?? "") &&
    (prev.marcadoresR16?.[id]?.local ?? "") === (next.marcadoresR16?.[id]?.local ?? "") &&
    (prev.marcadoresR16?.[id]?.visitante ?? "") === (next.marcadoresR16?.[id]?.visitante ?? "")
  )
}

export default React.memo(PartidoCardR16, areEqual)
