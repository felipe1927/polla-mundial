...
function PartidoCardSF({
  ...
  // pronosticosSF (prop del padre) es la única fuente de verdad.
  ...
  const pick = pronosticosSF?.[partido.id] ?? { local: "", visitante: "" }
  ...
  const handleAdminLocalChange = (e) => {
    const val = e.target.value.replace(/[^0-9]/g, "")
    const nuevoLocal = val === "" ? "" : parseInt(val, 10)
    const visitante = marcadoresSF?.[partido.id]?.visitante ?? ""
  ...
  const handleAdminVisitanteChange = (e) => {
    const val = e.target.value.replace(/[^0-9]/g, "")
    const nuevoVisitante = val === "" ? "" : parseInt(val, 10)
    const local = marcadoresSF?.[partido.id]?.local ?? ""
  ...
  const marcador = marcadoresSF?.[partido.id]
...
function areEqual(prev, next) {
  ...
  return (
    ...
    prev.enviadoSF === next.enviadoSF &&
    prev.sfCerrado === next.sfCerrado &&
    (prev.pronosticosSF?.[id]?.local ?? "") === (next.pronosticosSF?.[id]?.local ?? "") &&
    (prev.pronosticosSF?.[id]?.visitante ?? "") === (next.pronosticosSF?.[id]?.visitante ?? "") &&
    (prev.marcadoresSF?.[id]?.local ?? "") === (next.marcadoresSF?.[id]?.local ?? "") &&
    (prev.marcadoresSF?.[id]?.visitante ?? "") === (next.marcadoresSF?.[id]?.visitante ?? "")
  )
}

export default React.memo(PartidoCardSF, areEqual)
