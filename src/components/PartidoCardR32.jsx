...
function PartidoCardR32({
  ...
  const [pronosticoLocal, setPronosticoLocal] = React.useState(pronosticosR32?.[partido.id]?.local ?? "")
  const [pronosticoVisitante, setPronosticoVisitante] = React.useState(pronosticosR32?.[partido.id]?.visitante ?? "")
  
  // Actualizar inputs cuando carga pronosticosR32 desde Firebase
  React.useEffect(() => {
    setPronosticoLocal(pronosticosR32?.[partido.id]?.local ?? "")
    setPronosticoVisitante(pronosticosR32?.[partido.id]?.visitante ?? "")
  }, [pronosticosR32, partido.id])
  
  // Guardar en pronosticosR32 SOLO cuando AMBOS campos tienen valor
  React.useEffect(() => {
    if (pronosticoLocal !== "" && pronosticoVisitante !== "") {
      setPronosticosR32((prev) => ({
        ...prev,
        [partido.id]: {
  ...
  }, [pronosticoLocal, pronosticoVisitante, partido.id, setPronosticosR32])
  ...
  const handleAdminLocalChange = (e) => {
    const val = e.target.value.replace(/[^0-9]/g, "")
    const nuevoLocal = val === "" ? "" : parseInt(val, 10)
    const visitante = marcadoresR32?.[partido.id]?.visitante ?? ""
  ...
  const handleAdminVisitanteChange = (e) => {
    const val = e.target.value.replace(/[^0-9]/g, "")
    const nuevoVisitante = val === "" ? "" : parseInt(val, 10)
    const local = marcadoresR32?.[partido.id]?.local ?? ""
  ...
  const marcador = marcadoresR32?.[partido.id]
...
function areEqual(prev, next) {
  ...
  return (
    ...
    prev.enviadoR32 === next.enviadoR32 &&
    prev.r32Cerrado === next.r32Cerrado &&
    (prev.pronosticosR32?.[id]?.local ?? "") === (next.pronosticosR32?.[id]?.local ?? "") &&
    (prev.pronosticosR32?.[id]?.visitante ?? "") === (next.pronosticosR32?.[id]?.visitante ?? "") &&
    (prev.marcadoresR32?.[id]?.local ?? "") === (next.marcadoresR32?.[id]?.local ?? "") &&
    (prev.marcadoresR32?.[id]?.visitante ?? "") === (next.marcadoresR32?.[id]?.visitante ?? "")
  )
}

export default React.memo(PartidoCardR32, areEqual)
